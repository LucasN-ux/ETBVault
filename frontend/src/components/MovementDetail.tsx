...
import { detecterMouvement, NIVEAU_LABEL, type Horizon, type ReleveDePrix } from '../utils/mouvement'
import { Icon, type NomIcone } from './Icon'

function MvLine({ titre, mv }: { titre: string; mv: Horizon | null | undefined }) {
  ...
  const icon: NomIcone =
    indispo || mv.niveau === 'faible' ? 'flat' : dir === 'hausse' ? 'arrowUp' : dir === 'baisse' ? 'arrowDown' : 'flat'
  const pct =
...
export default function MovementDetail({ historique }: { historique: readonly ReleveDePrix[] }) {
