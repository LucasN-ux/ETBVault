...
import type { ReleveDePrix } from '../utils/mouvement'
import { Segmented, Price, VarNum } from './ui'
import type { OptionSegment } from './ui/Controls'

type Mode = 'jours' | 'mois'
type Source = 'cm' | 'ebay'

/** Point du graphe, quel que soit le mode. */
interface PointGraphe {
  x: string
  label: string
  prix: number
  /** Nombre de relevés agrégés — seulement en mode Mois. */
  n?: number
}
...
function CustomTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload: PointGraphe }> }) {
...
export default function PriceChart({ historique, prixSortie, height = 230 }: PriceChartProps) {
  const gid = useId().replace(/:/g, '')
  const [mode, setMode] = useState<Mode>('jours')
  const [source, setSource] = useState<Source>('cm')

  const data = useMemo<PointGraphe[]>(() => {
    if (source !== 'cm') return []
    return mode === 'jours'
