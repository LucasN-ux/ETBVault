...
import type { PointPrix } from '../types/domaine'
import type { HistoriqueParEtb } from '../utils/valeurHistorique'
...
export default function Vault() {
  ...
  // Le brouillon reste en chaînes : c'est ce que rendent les champs, et la
  // conversion est faite une seule fois par useVault.addEntry.
  ...
  const hist = useMemo<HistoriqueParEtb>(() => {
    const parEtb: HistoriqueParEtb = {}
    for (const [id, points] of Object.entries<PointPrix[]>(historiques.donnees ?? OBJET_VIDE)) {
  ...
  const rows = useMemo(() => entries.map((e) => {
    ...
    const ref = prixActuels[e.etbId]?.prixActuel ?? Number(etb?.prixSortie ?? e.prixAchat)
