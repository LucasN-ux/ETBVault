...
import type { Carte } from '../types/domaine'
...
function CardArt({ carte }: { carte: Carte }) {
  const [imgErr, setImgErr] = useState(false)
  const prix = Number(carte.prixMarche ?? 0)
  const hue = hueFromId(carte.id ?? carte.numero ?? '')
  // La rareté est absente pour les cartes que TCGdex ne renseigne pas.
  const rarete = carte.rarete ?? ''
  const foil = FOIL.has(rarete)
  const label = RARETE_LABEL[rarete] ?? rarete
...
export default function ETBDetail() {
  // La route est /etb/:id : le paramètre est toujours là. Le repli sur '' rend
  // le cas impossible visible (400 puis EtatErreur) plutôt que de le masquer.
  const { id = '' } = useParams()
  ...
  const [rarete, setRarete] = useState<string | null>(null)
  ...
  const prixSortie = etb ? Number(etb.prixSortie ?? 0) : 0
      ...
      <section className="mx-auto" style={{ maxWidth: 1180, padding: 'clamp(18px,3vw,28px) clamp(18px,4vw,40px)' }}>
        <div className="lg:grid lg:gap-8" style={{ gridTemplateColumns: '380px 1fr' }}>
          ...
          <div className="mt-8 lg:mt-0 flex flex-col" style={{ gap: 18 }}>
            <div>
              <EraTag>{etb.era} · {dateFr(etb.dateSortie)}</EraTag>
