...
import type { LigneTicker } from '../components/ui/Indicateurs'

type Periode = 7 | 30 | 90

/** Une ETB du classement : catalogue + variation + courbe. */
interface EtbClassee extends Etb {
  annee: number | ''
  prixActuel: number
...
const PERIODES: ReadonlyArray<{ value: Periode; label: string }> = [
  { value: 7, label: '7 j' },
  { value: 30, label: '30 j' },
  { value: 90, label: '3 mois' },
]

function prixSeries(series: readonly PointSparkline[] | undefined, n: number): number[] {
  return (series ?? []).slice(-n).map((p) => Number(p.cmPrixMoyen))
}

function HeroSlab({ etb, periode, go }: { etb: EtbClassee; periode: Periode; go: (chemin: string) => void }) {
...
function TrendRow({ etb, rang, go }: { etb: EtbClassee; rang: number; go: (chemin: string) => void }) {
  const up = (etb.variationPct ?? 0) >= 0
