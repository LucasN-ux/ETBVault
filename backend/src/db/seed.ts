...
// Catalogue des 66 ETB curées à la main. Idempotent (upsert).
// Usage : npm run seed
...
/** Segment de série dans l'arborescence des assets TCGdex. */
function seriesPath(setId: string): string {
...
function logoUrl(setId: string): string {
...
const CM_ETBS: Record<string, string> = {
  // ── Écarlate et Violet ─────────────────────────────────────────────────
